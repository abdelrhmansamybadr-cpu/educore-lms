...
export default function AdminAnnouncementsPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-announcements'] }); setShowCreate(false); setForm({ title: '', content: '', targetRole: '' }) },
  })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">{isRtl ? 'إدارة إعلانات المدرسة' : 'Manage school announcements'}</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-bold text-gray-900">{isRtl ? 'إضافة إعلان' : 'New Announcement'}</h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <textarea
                ...
                placeholder={isRtl ? 'نص الإعلان...' : 'Announcement content...'}
                rows={5}
              ...
              <select
                ...
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-primary-500"
              >
                <option value="">{isRtl ? 'الجميع' : 'Everyone'}</option>
                <option value="STUDENT">{isRtl ? 'الطلاب' : 'Students'}</option>
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => create.mutate({ title: form.title, content: form.content, targetRole: form.targetRole || undefined })}
                ...
                >
                  {create.isPending ? '...' : (isRtl ? 'نشر' : 'Publish')}
                </button>
              </div>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-gray-900">{isRtl ? 'الإعلانات المنشورة' : 'Published Announcements'}</h3>
          </div>
        </CardHeader>
        <CardBody>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-gray-400">
              ...
              <p>{isRtl ? 'لا توجد إعلانات' : 'No announcements yet'}</p>
            </div>
          ) : (
                ...
                <div key={ann.id} className="flex items-start justify-between gap-4 p-4 rounded-xl border border-gray-100 bg-gray-50">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                      ...
                      {ann.targetRole ? ` · ${ann.targetRole}` : ` · ${isRtl ? 'الجميع' : 'Everyone'}`}
                    </p>
                  </div>
                  <button
                    onClick={() => { if (confirm(isRtl ? 'حذف الإعلان؟' : 'Delete announcement?')) deleteAnn.mutate(ann.id) }}
